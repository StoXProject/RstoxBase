...
ImputeDataByRandomSampling_StoX3 <- function(
    ...
    if(any(duplicatedIndividual)) {
        warning("StoX: There are duplicated entries in the Individual column which imples that individuals were used in multiple Strata. Due to a bug in the deprecated ImputeSuperIndividuals_StoX3() this may result in non-imputed rows. When you see this warning it is advised to use ImputeDataByRandomSampling() instead.")
    }
